...
- AR-H3：将 predictor 连续调用三次，每一步把上一步预测结果放回上下文。

C1 的 H3 action sequence 为：

```text
当前 query 的 g2 → 下一 query 的 g0 → 下一 query 的 g1
```

H3 是冻结 predictor 的自回归滚动，不是单独训练的多步 prediction head。
...
| F2 | strict causal，双视角，C1（当前 z2） | 正确 action 的自回归序列（g2→下一 query 的 g0→g1） | strict causal 连续未来 latent（z3,z4,z5） | 检验多步滚动预测 |
